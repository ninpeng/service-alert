    ...
    <section className="content incident-history-content">
      ...
      <section className="incident-filter-band" aria-label="장애 이력 필터">
        <nav className="segmented-control" aria-label="진행 상태">
          {stateOptions.map(([value, label]) => <a key={value} className={filters.state === value ? "active" : undefined} href={buildIncidentSearchHref(filters, { state: value, page: 1 })} aria-current={filters.state === value ? "page" : undefined}>{label}</a>)}
        </nav>
        <form className="incident-filter-form" method="get" action="/incidents">
          ...
          <div className="incident-filter-actions"><button className="command-button" type="submit"><Search aria-hidden="true" size={16} />조회</button><a className="secondary-command" href="/incidents"><RotateCcw aria-hidden="true" size={16} />초기화</a></div>
        </form>
      </section>
      <section className="panel incident-results">
        ...
        {data.incidents.length > 0 ? <IncidentTable incidents={data.incidents} /> : <div className="empty-state incident-empty"><strong>조건에 맞는 장애 이력이 없습니다.</strong><a href="/incidents">기본 조건으로 돌아가기</a></div>}
